...
import { getDepositList } from '@/api/admin/depositJudge/api';
import { DEPOSIT_LIST, DEPOSIT_LIST_REQUEST } from '@/api/admin/depositJudge/model';
...
const Page = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: ({ queryKey }) => getDepositList(queryKey[1] as DEPOSIT_LIST_REQUEST),
  });

  const column: ColumnDef<DEPOSIT_LIST>[] = [
    {
      accessorKey: 'id',
      header: 'No.',
    },
